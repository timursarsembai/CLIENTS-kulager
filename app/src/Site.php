...
final class Site
{
    ...
    /**
     * Метка места, куда подставится разовый номер для встроенных скриптов.
     *
     * Сам номер в разметку не пишем: готовая страница кладётся в кэш и
     * отдаётся многим посетителям, а номер обязан быть у каждого свой —
     * иначе он ничего не значит. Поэтому в HTML идёт метка, а подмена
     * происходит перед самой отдачей, в том числе и для страницы из кэша.
     */
    public const NONCE_MARK = '{{nonce}}';

    /** Готовый атрибут для тега: nonce="…". */
    public function nonceAttr(): string
    {
        return 'nonce="' . self::NONCE_MARK . '"';
    }
