...
final class Leads
{
    ...
    public function accept(array $input, string $page, string $locale): array
    {
        ...
        /*
         * Согласие на обработку персональных данных. Без него заявку не
         * принимаем: собирать имя и телефон без согласия человека нельзя,
         * а «мелким шрифтом под кнопкой» согласие потом не докажешь.
         */
        if (($input['consent'] ?? '') !== '1') {
            return ['ok' => false, 'error' => 'Отметьте согласие на обработку персональных данных.'];
        }
        ...
        $id = $this->db->insert('leads', [
            ...
            'email'      => $email,
            'message'    => $message,
            // Текст согласия храним целиком: формулировка на сайте со временем
            // меняется, а подтверждать придётся ту, что человек видел тогда
            'consent'      => 1,
            'consent_text' => $this->clean($input['consent_text'] ?? '', 500),
